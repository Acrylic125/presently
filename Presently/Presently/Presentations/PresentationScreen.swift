  ...
  var body: some View {
    tokens.reduce(Text("")) { partialResult, token in
      partialResult
        + Text(token.text)
        .foregroundColor(
          token.type == .highlight ? AppColors.Primary500.color : AppColors.Gray50.color)
    }
...
struct PresentationToolbar<Content: View>: View {
  ...
  init(
    toolbarAppearTransitionState: Double,
...
struct PresentationView: View {
  ...
  var body: some View {
    ...
    .onAppear {
      speechRecognizer.numberOfAudioAmplitudes = horizontalSizeClass == .regular ? 30 : 15
    }
  }
